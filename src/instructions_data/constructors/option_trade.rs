...
impl<'a> DvlInstructionData<'a> for InstructionOptionTrade {
    ...
    fn new(params: Self::DvlInstrParams) -> Result<Box<InstructionOptionTrade>, Box<dyn Error>> {
        ...
        let mut basket_array =
            [OptionTradeBasketData { strike: 0, put_or_call: PutOrCall::PUT, amount: 0 }; INSTR_OPTION_TRADE_MAX_BASKET_LENGTH];
...
impl Default for InstructionOptionTrade {
    fn default() -> Self {
        InstructionOptionTrade {
            ...
            max_cost: 0,
            basket: [OptionTradeBasketData::default(); 4],
        }
    }
}
...
mod tests {
    ...
    fn test_filled_basket_instruction_option_trade() {
        let custom_basket_data = [
            OptionTradeBasketData { strike: 10, put_or_call: PutOrCall::PUT, amount: -30 },
            OptionTradeBasketData { strike: 40, put_or_call: PutOrCall::CALL, amount: -60 },
            OptionTradeBasketData { strike: 70, put_or_call: PutOrCall::PUT, amount: -90 },
        ];
        let trade_params = OptionTradeParams {
            trade_qty: [0; BUCKETS_COUNT],
