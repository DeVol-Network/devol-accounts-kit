...
mod tests {
    ...
    fn test_worker_account_offsets() {
        ...
        assert_eq!(unsafe { &account.header.tag as *const _ as usize } - base_ptr, WORKER_ACCOUNT_TAG_OFFSET);
        assert_eq!(unsafe { &account.header.version as *const _ as usize } - base_ptr, WORKER_ACCOUNT_VERSION_OFFSET);
        assert_eq!(unsafe { &account.header.root as *const _ as usize } - base_ptr, WORKER_ACCOUNT_ROOT_ADDRESS_OFFSET);
        assert_eq!(unsafe { &account.id as *const _ as usize } - base_ptr, WORKER_ACCOUNT_ID_OFFSET);
        assert_eq!(unsafe { &account.state as *const _ as usize } - base_ptr, WORKER_ACCOUNT_STATE_OFFSET);
        assert_eq!(unsafe { &account.task_id as *const _ as usize } - base_ptr, WORKER_ACCOUNT_TASK_ID_OFFSET);
        assert_eq!(unsafe { &account.instr_id as *const _ as usize } - base_ptr, WORKER_ACCOUNT_INSTR_ID_OFFSET);
        assert_eq!(unsafe { &account.ops_counter as *const _ as usize } - base_ptr, WORKER_ACCOUNT_OPS_COUNTER_OFFSET);
        assert_eq!(unsafe { &account.first_time as *const _ as usize } - base_ptr, WORKER_ACCOUNT_FIRST_TIME_OFFSET);
        assert_eq!(unsafe { &account.duration as *const _ as usize } - base_ptr, WORKER_ACCOUNT_DURATION_OFFSET);
        assert_eq!(unsafe { &account.init_px as *const _ as usize } - base_ptr, WORKER_ACCOUNT_INIT_PX_OFFSET);
        assert_eq!(unsafe { &account.own_ps as *const _ as usize } - base_ptr, WORKER_ACCOUNT_OWN_PS_OFFSET);
        assert_eq!(unsafe { &account.width_factor as *const _ as usize } - base_ptr, WORKER_ACCOUNT_WIDTH_FACTOR_OFFSET);
        assert_eq!(unsafe { &account.fee_rate as *const _ as usize } - base_ptr, WORKER_ACCOUNT_FEE_RATE_OFFSET);
        assert_eq!(unsafe { &account.fee_ratio as *const _ as usize } - base_ptr, WORKER_ACCOUNT_FEE_RATIO_OFFSET);
        assert_eq!(unsafe { &account.inventories_ratio as *const _ as usize } - base_ptr, WORKER_ACCOUNT_INVENTORIES_RATIO_OFFSET);
        assert_eq!(unsafe { &account.fractions as *const _ as usize } - base_ptr, WORKER_ACCOUNT_FRACTIONS_OFFSET);
        assert_eq!(unsafe { &account.max_order as *const _ as usize } - base_ptr, WORKER_ACCOUNT_MAX_ORDER_OFFSET);
        assert_eq!(unsafe { &account.new_width_factor as *const _ as usize } - base_ptr, WORKER_ACCOUNT_NEW_WIDTH_FACTOR_OFFSET);
        assert_eq!(unsafe { &account.new_fee_rate as *const _ as usize } - base_ptr, WORKER_ACCOUNT_NEW_FEE_RATE_OFFSET);
        assert_eq!(unsafe { &account.new_fee_ratio as *const _ as usize } - base_ptr, WORKER_ACCOUNT_NEW_FEE_RATIO_OFFSET);
        assert_eq!(unsafe { &account.new_inventories_ratio as *const _ as usize } - base_ptr, WORKER_ACCOUNT_NEW_INVENTORIES_RATIO_OFFSET);
        assert_eq!(unsafe { &account.new_fractions as *const _ as usize } - base_ptr, WORKER_ACCOUNT_NEW_FRACTIONS_OFFSET);
        assert_eq!(unsafe { &account.new_max_order as *const _ as usize } - base_ptr, WORKER_ACCOUNT_NEW_MAX_ORDER_OFFSET);
        assert_eq!(unsafe { &account.new_max_inventories as *const _ as usize } - base_ptr, WORKER_ACCOUNT_NEW_MAX_INVENTORIES_OFFSET);
        assert_eq!(unsafe { &account.pool_time as *const _ as usize } - base_ptr, WORKER_ACCOUNT_POOL_TIME_OFFSET);
        assert_eq!(unsafe { &account.lp_counter as *const _ as usize } - base_ptr, WORKER_ACCOUNT_LP_COUNTER_OFFSET);
        assert_eq!(unsafe { &account.last_lp_time as *const _ as usize } - base_ptr, WORKER_ACCOUNT_LAST_LP_TIME_OFFSET);
        assert_eq!(unsafe { &account.pool_size as *const _ as usize } - base_ptr, WORKER_ACCOUNT_POOL_SIZE_OFFSET);
        assert_eq!(unsafe { &account.cps as *const _ as usize } - base_ptr, WORKER_ACCOUNT_CPS_OFFSET);
        assert_eq!(unsafe { &account.cps_px as *const _ as usize } - base_ptr, WORKER_ACCOUNT_CPS_PX_OFFSET);
        assert_eq!(unsafe { &account.cps_in_ps as *const _ as usize } - base_ptr, WORKER_ACCOUNT_CPS_IN_PS_OFFSET);
        assert_eq!(unsafe { &account.ps as *const _ as usize } - base_ptr, WORKER_ACCOUNT_PS_OFFSET);
        assert_eq!(unsafe { &account.ps_px as *const _ as usize } - base_ptr, WORKER_ACCOUNT_PS_PX_OFFSET);
        assert_eq!(unsafe { &account.task_fees as *const _ as usize } - base_ptr, WORKER_ACCOUNT_TASK_FEES_OFFSET);
        assert_eq!(unsafe { &account.rest_of_fees as *const _ as usize } - base_ptr, WORKER_ACCOUNT_REST_OF_FEES_OFFSET);
        assert_eq!(unsafe { &account.pool_id as *const _ as usize } - base_ptr, WORKER_ACCOUNT_POOL_ID_OFFSET);
        assert_eq!(unsafe { &account.start as *const _ as usize } - base_ptr, WORKER_ACCOUNT_START_OFFSET);
        assert_eq!(unsafe { &account.expiration as *const _ as usize } - base_ptr, WORKER_ACCOUNT_EXPIRATION_OFFSET);
        assert_eq!(unsafe { &account.pool_cashflow as *const _ as usize } - base_ptr, WORKER_ACCOUNT_POOL_CASHFLOW_OFFSET);
        assert_eq!(unsafe { &account.settlement_px as *const _ as usize } - base_ptr, WORKER_ACCOUNT_SETTLEMENT_PX_OFFSET);
        assert_eq!(unsafe { &account.pool_depo as *const _ as usize } - base_ptr, WORKER_ACCOUNT_POOL_DEPO_OFFSET);
        assert_eq!(unsafe { &account.pool_fees as *const _ as usize } - base_ptr, WORKER_ACCOUNT_POOL_FEES_OFFSET);
        assert_eq!(unsafe { &account.pool_counter as *const _ as usize } - base_ptr, WORKER_ACCOUNT_POOL_COUNTER_OFFSET);
        assert_eq!(unsafe { &account.pool_strikes as *const _ as usize } - base_ptr, WORKER_ACCOUNT_POOL_STRIKES_OFFSET);
        assert_eq!(unsafe { &account.pool_bounds as *const _ as usize } - base_ptr, WORKER_ACCOUNT_POOL_BOUNDS_OFFSET);
        assert_eq!(unsafe { &account.pool_distrib as *const _ as usize } - base_ptr, WORKER_ACCOUNT_POOL_DISTRIB_OFFSET);
        assert_eq!(unsafe { &account.svm_params as *const _ as usize } - base_ptr, WORKER_ACCOUNT_SVM_PARAMS_OFFSET);
        assert_eq!(unsafe { &account.margin_vega as *const _ as usize } - base_ptr, WORKER_ACCOUNT_MARGIN_VEGA_OFFSET);
        assert_eq!(unsafe { &account.margin_vanna as *const _ as usize } - base_ptr, WORKER_ACCOUNT_MARGIN_VANNA_OFFSET);
        assert_eq!(unsafe { &account.margin_volga as *const _ as usize } - base_ptr, WORKER_ACCOUNT_MARGIN_VOLGA_OFFSET);
        assert_eq!(unsafe { &account.range_lr as *const _ as usize } - base_ptr, WORKER_ACCOUNT_RANGE_LR_OFFSET);
        assert_eq!(unsafe { &account.w_lr as *const _ as usize } - base_ptr, WORKER_ACCOUNT_W_LR_OFFSET);
        assert_eq!(unsafe { &account.max_lr as *const _ as usize } - base_ptr, WORKER_ACCOUNT_MAX_LR_OFFSET);
        assert_eq!(unsafe { &account.max_pct_pool as *const _ as usize } - base_ptr, WORKER_ACCOUNT_MAX_PCT_POOL_OFFSET);
        assert_eq!(unsafe { &account.reserved as *const _ as usize } - base_ptr, WORKER_ACCOUNT_RESERVED_OFFSET);
