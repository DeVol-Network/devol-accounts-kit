...
impl DvlReadable for TasksLogAccount {
    ...
    fn get_public_key<'a>(client: &DvlClient, params: &DvlIndexParam) -> Result<Box<Pubkey>, Box<dyn Error>> where Self: Sized {
...
mod tests {
    ...
    fn test_read_tasks_log_account_by_index() -> Result<(), Box<dyn Error>> {
        ...
        let _task_log_0 = client.get_account::<TasksLogAccount>(DvlIndexParam { id: 0 })?;
        Ok(())
    }
    ...
    fn test_read_tasks_log_account_by_public_key() -> Result<(), Box<dyn Error>> {
        ...
        let _task_log_0 = client.get_account_by_public_key::<TasksLogAccount>(pubkey)?;
        Ok(())
    }
}
